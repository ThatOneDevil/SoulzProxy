    ...
    override fun execute(p0: SimpleCommand.Invocation?) {
        ...
        if (source != null) {
            val message = Component.text()
                ...
                .append(convertLegacyToMiniMessage(" <color:#EF5B80><bold>▍ &r<color:#EF5B80>Proxy Version: <color:#F11642>${proxy.version.version}\n"))
                .append(convertLegacyToMiniMessage(" <color:#EF5B80><bold>▍ &r<color:#EF5B80>Number of Players: <color:#F11642>${proxy.playerCount}\n"))
                .append(convertLegacyToMiniMessage(" <color:#EF5B80><bold>▍ &r<color:#EF5B80>Server Count: <color:#F11642>${servers.size}\n\n"))
                .append(convertLegacyToMiniMessage("&r<color:#F11642>=== Servers List ===\n"))

            for (server in servers) {
                val playerCount = server.playersConnected.size
                message.append(convertLegacyToMiniMessage(" <color:#EF5B80><bold>▍ ${server.serverInfo.name}:</bold> <color:#F11642>$playerCount\n"))
            }
